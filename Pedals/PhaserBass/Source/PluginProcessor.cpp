...
juce::AudioProcessorValueTreeState::ParameterLayout PhaserBassAudioProcessor::createParameterLayout()
{
...
PhaserBassAudioProcessor::PhaserBassAudioProcessor()
...
void PhaserBassAudioProcessor::prepareToPlay (double sampleRateIn, int samplesPerBlock)
{
...
void PhaserBassAudioProcessor::releaseResources()
{
}

bool PhaserBassAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
  #if JucePlugin_IsMidiEffect
...
void PhaserBassAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    constexpr float fMin = 80.0f;
    constexpr float fMax = 900.0f;
    const float logMin = std::log (fMin);
    const float logMax = std::log (fMax);

    const int channelsToProcess = juce::jmin (totalNumInputChannels, (int) PhaserBassAudioProcessor::maxChannels);
...
juce::AudioProcessorEditor* PhaserBassAudioProcessor::createEditor()
{
    return new juce::GenericAudioProcessorEditor(*this);
}

void PhaserBassAudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
...
void PhaserBassAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new PhaserBassAudioProcessor();
}
