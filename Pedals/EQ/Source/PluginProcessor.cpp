...
void EQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    coeffs[0] = juce::dsp::IIR::Coefficients<float>::makeLowShelf (
        currentSampleRate,
    ...
    for (int i = 1; i <= 4; ++i)
    {
        ...
        coeffs[(size_t) i] = juce::dsp::IIR::Coefficients<float>::makePeakFilter (
            ...
            juce::Decibels::decibelsToGain (apvts.getRawParameterValue (prefix + "GAIN")->load()));
    }

    coeffs[5] = juce::dsp::IIR::Coefficients<float>::makeHighShelf (
        currentSampleRate,
