...
juce::AudioProcessorValueTreeState::ParameterLayout ChorusBassAudioProcessor::createParameterLayout()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;

    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID { "RATE", 1 }, "Rate",
        juce::NormalisableRange<float> { 0.05f, 5.0f, 0.0f, 0.4f }, 0.4f));

    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID { "DEPTH", 1 }, "Depth", 0.0f, 1.0f, 0.22f));
...
ChorusBassAudioProcessor::ChorusBassAudioProcessor()
...
void ChorusBassAudioProcessor::prepareToPlay (double sampleRateIn, int samplesPerBlock)
{
...
void ChorusBassAudioProcessor::releaseResources()
{
}

bool ChorusBassAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
  #if JucePlugin_IsMidiEffect
...
void ChorusBassAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int sample = 0; sample < numSamples; ++sample)
    {
        ...
        for (int channel = 0; channel < totalNumInputChannels; ++channel)
        {
            ...
            for (int v = 0; v < numVoices; ++v)
            {
                ...
                const float modMs = depth * ChorusBassAudioProcessor::maxModMs
...
juce::AudioProcessorEditor* ChorusBassAudioProcessor::createEditor()
{
    return new juce::GenericAudioProcessorEditor (*this);
}

void ChorusBassAudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
...
void ChorusBassAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new ChorusBassAudioProcessor();
}
